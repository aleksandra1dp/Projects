...
public:
  ...
  //          If the tree is empty or if no element is greater than
  //          the given value, returns an end Iterator.
  ...
  // EFFECTS : Inserts the element k into this BinarySearchTree, maintaining the
  //           sorting invariant. Returns an iterator to the newly inserted element.
...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static bool empty_impl(Node* root) {
    return root == nullptr;
  }
  ...
  static int height_impl(const Node *node) {
    ...
    int height_right = height_impl(node->right);
  ...
  // NOTE:    This function must be tree recursive.
    ...
    if (node->left == nullptr && node->right == nullptr) // leaf node
      return true;
    if (node->left == nullptr) //right has branch
      return check_sorting_invariant_impl(node->right, less) &&
       less(node->datum, min_element_impl(node->right)->datum);

    else
    if (node->right == nullptr) //left has branch
      return less(max_element_impl(node->left)->datum, node->datum)&&
       check_sorting_invariant_impl(node->left, less);

    else //both have branch
      return less(node->datum, min_element_impl(node->right)->datum) &&
       check_sorting_invariant_impl(node->left, less) &&
  ...
  static Node * min_greater_than_impl(Node *node, const T &val, Compare less) {
    ...
    if (!less(val, node->datum)) { // current value is less than val, look right
      return min_greater_than_impl(node->right, val, less);
    }
    Node* smallest = min_greater_than_impl(node->left, val, less);
    if (smallest != nullptr) {
      return smallest;
    }
    return node; // no smaller value
  }
